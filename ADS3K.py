...
# load the dataset
df = pd.read_csv(r"C:\Users\navee\Downloads\API_EG.USE.ELEC.KH.PC_DS2_en_csv_v2_5359189.csv", header=2)
...
plt.title('Electric power consumption Results')
plt.xlabel('Country Index')
plt.ylabel('Electric power consumption (kWh per capita) in 2000')
...
plt.title(f'{country} Electric power consumption Fitting')
plt.legend()
plt.show()
...
# Get the electric consumption data from 2000 to 2010
years = [str(year) for year in range(2000, 2011)]
electric_consumption = [df_country[year].values[0] for year in years]

# Create the line chart
plt.plot(years, electric_consumption, marker='o', color='#8ea594')
plt.title(f'{country}: Electric Power Consumption from 2000 to 2010')
plt.xlabel('Year')
